...
class CodeFlow {
  ...
  public GoogleCredential serviceAccount = null;
  ...
  public void setSecrets() {
    try {
      secrets = GoogleClientSecrets.load(JSON_FACTORY, new InputStreamReader(CodeFlow.class.getResourceAsStream("client_secrets.json")));
    }
    catch(IOException e) {
  ...
  public void setServiceAccount() {
    try {
      log.finer("setServiceAccount");
      String appEngineEnvironment = System.getProperty("com.google.appengine.runtime.environment");
      if (/*(appEngineEnvironment != null) &&*/ (serviceAccount == null) && (CodeFlow.class.getResource("service_account.json") != null)) {
        log.finer("Service account config exists, attempting to use service account");
        serviceAccount = GoogleCredential.fromStream(CodeFlow.class.getResourceAsStream("service_account.json")).createScoped(Collections.singleton(FusiontablesScopes.FUSIONTABLES));
        log.finer("Service account set");
      }
    }
    catch(IOException e) {
      log.severe(e.getMessage());
  ...
  public Credential getCredential() throws IOException {
    if(serviceAccount != null) {
      ...
      return this.build().loadCredential("administrator");
    }
  }

  public boolean authorized() throws IOException {
    if((serviceAccount == null) && (secrets == null)){
      return false;
    }
    else {
